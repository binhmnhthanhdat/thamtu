<?php 
    ...
    function laytinmoi_alias_pt($alias,$page,$limit){
     ...
     function tagtin($alias){
		$sql="select * from news where type!=2 and alias like \"%".$alias."%\" or description like \"%".$alias."%\" or content like \"%".$alias."%\"";//echo $sql; exit;	
	//	echo $sql;exit;
        return $this->db->query($sql);
		
	} 
	function tagtin_pt($alias,$page,$limit){
		$sql="select * from news where type!=2 and alias like \"%".$alias."%\" or description like \"%".$alias."%\" or content like \"%".$alias."%\"  limit $page,$limit ";//echo $sql; exit;	
	    return $this->db->query($sql);
